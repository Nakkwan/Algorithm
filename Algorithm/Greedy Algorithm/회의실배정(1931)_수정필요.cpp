...
int partition(vector<vector<int>> list, int left, int right) { //list1 is list for start_time, list2 is list for finish time
	int pivot, temp;
	int low, high;
...
void quick_sort(vector<vector<int>> list, int left, int right) {
...
int main() {
	...
	int count = 1;
	cin >> N;			
	vector<vector<int>> list_time(N, vector<int>(2));

	//int** list_time;		//Dynamic array for conference time
	//list_time = new int*[100000];
	//for (int i = 0; i < N; i++) {
	//	list_time[i] = new int[2];		//each low of list has two element about start time and finish time
	//}
